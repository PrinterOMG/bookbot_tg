...
from utils.db_api import UsersWorker, LanguagesWorker, ReferralWorker, SubPricesWorker, PromocodesWorker, \
    SettingsWorker, PostWorker, FilterWorker, StatisticWorker, ArchiveWorker
...
archive_worker = ArchiveWorker(db_name=config.DB_NAME, password=config.DB_PASSWORD, username=config.DB_USER, host=config.DB_HOST)
